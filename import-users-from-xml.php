	...
	public function process_xml() {
		if ( isset( $_POST['_wpnonce-is-iu-import-users-users-page_import'] ) ) {
			...
			if ( isset( $_FILES['users_csv']['tmp_name'] ) ) {
				...
				$filename              = $_FILES['users_xml']['tmp_name'];
...
<div class="wrap">
	...
	if ( ! file_exists( $error_log_file ) ) {
		if ( ! @fopen( $error_log_file, 'x' ) )
			echo '<div class="updated"><p><strong>' . sprintf( __( 'Notice: please make the directory %s writable so that you can see the error log.' , 'import-users-from-xml'), self::$log_dir_path ) . '</strong></p></div>';
	}
	...
	if ( isset( $_GET['import'] ) ) {
		...
		switch ( $_GET['import'] ) {
			case 'file':
				echo '<div class="error"><p><strong>' . __( 'Error during file upload.' , 'import-users-from-xml') . '</strong></p></div>';
				break;
			case 'data':
				echo '<div class="error"><p><strong>' . __( 'Cannot extract data from uploaded file or no file was uploaded.' , 'import-users-from-xml') . '</strong></p></div>';
				break;
			case 'fail':
				echo '<div class="error"><p><strong>' . sprintf( __( 'No user was successfully imported%s.' , 'import-users-from-xml'), $error_log_msg ) . '</strong></p></div>';
				break;
			case 'errors':
				echo '<div class="error"><p><strong>' . sprintf( __( 'Some users were successfully imported but some were not%s.' , 'import-users-from-xml'), $error_log_msg ) . '</strong></p></div>';
				break;
			case 'success':
				echo '<div class="updated"><p><strong>' . __( 'Users import was successful.' , 'import-users-from-xml') . '</strong></p></div>';
				break;
			default:
				break;
		}
	...
	<form method="post" action="" enctype="multipart/form-data">
		...
		<table class="form-table">
			<tr valign="top">
				<th scope="row"><label for"users_csv"><?php _e( 'CSV file' , 'import-users-from-xml'); ?></label></th>
		...
		<p class="submit">
			<input type="submit" class="button-primary" value="<?php _e( 'Import' , 'import-users-from-xml'); ?>" />
		</p>
	</form>
<?php
	}
	...
	public static function import_xml( $filename, $password_nag = false, $new_user_notification = false ) {
		...
		//include( plugin_dir_path( __FILE__ ) . 'class-readxml.php' );
	...
	private static function log_errors( $errors ) {
		...
		$log = @fopen( self::$log_dir_path . 'xml_import_errors.log', 'a' );
		@fwrite( $log, sprintf( __( 'BEGIN %s' , 'import-users-from-xml'), date( 'Y-m-d H:i:s', time() ) ) . "\n" );

		foreach ( $errors as $key => $error ) {
			$line = $key + 1;
			$message = $error->get_error_message();
			@fwrite( $log, sprintf( __( '[Line %1$s] %2$s' , 'import-users-from-xml'), $line, $message ) . "\n" );
		}
